...
백엔드가 새 활성 스트림의 첫 추론 전에 AI 추적기를 초기화합니다. 수동 초기화 도구도 사용할 수 있습니다.
...
새 영상에는 새로운 `streamId`를 사용해야 합니다. 백엔드는 활성 스트림이 바뀌면 추론 직전 AI의 `POST /reset`을 호출합니다. 재요청 응답 조회와 스트림 종료에는 reset을 호출하지 않습니다. 백엔드 재시작 후 첫 신규 추론에도 reset이 수행되므로 새 streamId를 사용하세요. AI를 재시작한 경우에도 새 streamId가 필요합니다.
