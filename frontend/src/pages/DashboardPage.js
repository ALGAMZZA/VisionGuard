...
const cameraDefinitions = [
  { id: 1, cameraId: 'camera-1', name: 'CCTV 01', zone: 'ZONE_01', location: '자재 적재 구역', snapshotUrl: 'http://127.0.0.1:8080/snapshot.jpg' },
  { id: 2, cameraId: 'camera-2', name: 'CCTV 02', zone: 'ZONE_02', location: '제품 이동 통로', snapshotUrl: 'http://127.0.0.1:8081/snapshot.jpg' },
  { id: 3, cameraId: 'camera-3', name: 'CCTV 03', zone: 'ZONE_03', location: '창고 통로 구역', snapshotUrl: 'http://127.0.0.1:8083/snapshot.jpg' },
  { id: 4, cameraId: 'camera-4', name: 'CCTV 04', zone: 'ZONE_04', location: '적재 작업 구역', snapshotUrl: 'http://127.0.0.1:8084/snapshot.jpg' },
];
...
function DashboardPage() {
  ...
  const cameras = useMemo(() => cameraDefinitions.map((definition) => (
    analyses[definition.cameraId]
      ? cameraFromAnalysis(definition, analyses[definition.cameraId])
  ...
  useEffect(() => {
    ...
    async function refreshDashboard() {
      const results = await Promise.allSettled(
        cameraDefinitions.map((camera) => getLatestAnalysis(camera.cameraId)),
      );
      if (!active) return;

      const next = {};
      results.forEach((result, index) => {
        if (result.status === 'fulfilled') next[cameraDefinitions[index].cameraId] = result.value;
      });
      setAnalyses((current) => ({ ...current, ...next }));
      setBackendConnected(results.some((result) => (
        result.status === 'fulfilled' || Boolean(result.reason?.response)
      )));
    }

    refreshDashboard();
    const timer = window.setInterval(refreshDashboard, 1000);
  ...
  useEffect(() => {
    ...
    async function refreshAlerts() {
      try {
        const page = await getRiskEvents({ page: 0, size: 3 });
        if (active) setRecentAlerts((page.content || []).map(alertFromEvent));
      } catch (_) {}
    }
    refreshAlerts();
